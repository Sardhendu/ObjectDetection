...
class Train():
    ...
    def get_network_inputs(self):
        ...
        self.gt_masks = tf.placeholder(dtype=tf.float32,
                                  ...
                                  name='batch_gt_masks')
        self.gt_class_ids = tf.placeholder(dtype=tf.float32,
        ...
        self.anchors = tf.placeholder(dtype=tf.float32, shape=[None, None, 4],
                                 name="input_anchors")

        # CREATE BATCH DATA: Needed to create the graph for Detection Targets
        self.input_gt_class_ids = tf.placeholder(dtype=tf.float32,
    ...
    def build_train_graph(self):
        ...
        # GET GRAPH INPUTS
        # (xIN, gt_masks, gt_class_ids, gt_bboxes, rpn_target_class, rpn_target_bbox, anchors, input_gt_class_ids, input_gt_bboxes) = \
        #
    ...
    def exec_sess(self, data_dict, image_ids):
        ...
        with tf.Session() as sess:
            ...
            rpn_class_loss_ = sess.run(rpn_class_loss,
                            feed_dict={
                                self.rpn_target_class: batch_rpn_target_class, rpn_pred_logits:rpn_pred_logits_
                            })
