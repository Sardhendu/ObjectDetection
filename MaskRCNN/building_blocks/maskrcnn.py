...
class MaskRCNN():
    def __init__(self, image_shape, pool_shape, num_classes, levels, proposals, type='keras',
                 DEBUG=False):
        '''
        ...
        feature_maps = [self.P2, self.P3, self.P4, self.P5]
...
def debugg():
    ...
    rpn_probs = np.array(np.random.random((num_batches, 6, 2)), dtype='float32')
    rpn_bbox = np.array(np.random.random((num_batches, 6, 4)), dtype='float32')
    input_anchors = np.array(np.random.random((num_batches, 6, 4)), dtype='float32')
    ...
    obj_MRCNN = MaskRCNN(image_shape=[1024, 1024, 3], pool_shape=[7, 7], num_classes=81, levels=[2, 3, 4, 5],
                         proposals=p_graph['proposals'], type='keras', DEBUG=True)
    ...
    with tf.Session() as sess:
        ...
        (proposals_, pooled_rois_, roi_level_, box_to_level_, sorting_tensor_, ix_, FC1_, FC2_, shared_,
         mrcnn_class_probs_, mrcnn_bbox_)  =  sess.run([p_graph['proposals'], pooled_rois, roi_level, box_to_level,
                                                        sorting_tensor, ix, FC1, FC2, shared, mrcnn_graph[
                                                                    'mrcnn_class_probs'],
