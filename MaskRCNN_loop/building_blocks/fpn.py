...
class FPN():
    ...
    def build(self):
        C1, C2, C3, C4, C5 = self.fpn_bottom_up_graph()
        self.fpn_top_down_graph(C2, C3, C4, C5)
    ...
    def fpn_bottom_up_graph(self):
        ...
        print('(FPN) C1: (shape) ', C1.shape)
        print('(FPN) C2: (shape) ', C2.shape)
        print('(FPN) C3: (shape) ', C3.shape)
        print('(FPN) C4: (shape) ', C4.shape)
        print('(FPN) C5: (shape) ', C5.shape)
        
        return C1, C2, C3, C4, C5
        
    def fpn_top_down_graph(self, C2, C3, C4, C5):
        '''
        ...
        # Feature Map 1
        M5 = ops.conv_layer(C5, [1, 1, C5.get_shape().as_list()[-1], 256], stride=1, padding='SAME',
                            scope_name='fpn_c5p5')  # to reduce the channel depth
        logging.info('FPN - M5: %s', str(M5.get_shape().as_list()))
    
        # Feature Map 2
        m4_c = ops.conv_layer(C4, [1,1,C4.get_shape().as_list()[-1], 256], stride=1, padding='SAME',
                              scope_name='fpn_c4p4')
        ...
        # Feature Map 3
        m3_c = ops.conv_layer(C3, [1, 1, C3.get_shape().as_list()[-1], 256], stride=1, padding='SAME',
                              scope_name='fpn_c3p3')
        ...
        # Feature Map 4
        m2_c = ops.conv_layer(C2, [1, 1, C2.get_shape().as_list()[-1], 256], stride=1, padding='SAME',
                              scope_name='fpn_c2p2')
    ...
    def get_feature_maps(self):
        return [self.P2, self.P3, self.P4, self.P5, self.P6]
